...
console.log("Deployment:", url, token ? "(có token)" : "(KHÔNG có token — sẽ bị từ chối nếu backend yêu cầu đăng nhập)");
...
if (r2.denied) {
  console.log(`[classify] ✅ đã bị chặn khi thiếu botKey ("${String(r2.message).slice(0, 80)}") — đúng chính sách mới`);
} else {
  console.log(
