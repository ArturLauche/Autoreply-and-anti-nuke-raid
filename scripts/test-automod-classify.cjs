...
const PUNISH_EVENTS = new Set([
  "bot_writes:botRecordAntinukeEvent",
...
const store = {
    ...
    return {
      ...
      modules: [
        ...
        },
        {
          ...
          windowSeconds: 10,
          threshold: 1,
          actions: ["timeout", "deleteMessages"],
        },
        {
          module: "mention",
          enabled: true,
  ...
  client: {
    ...
    async mutation(name, args) {
      ...
      if (
        PUNISH_EVENTS.has(name) ||
        name.endsWith("AntinukeEvent") ||
        name.endsWith("RecordPunishment")
      ) {
...
(async () => {
  ...
  const v1 = await runScan(
    makeMessage({ content: "vào server mình nè https://discord.gg/abc123", authorId: USER_A }),
  );
  ...
  const v3 = await runScan(
    makeMessage({
      content: "nhận nitro miễn phí tại https://discord-nitro.ru/free",
      authorId: USER_A,
    }),
  );
  check("domain scam trong blacklist → phạt", v3.length === 1 && v3[0].module === "malware");

  const v4 = await runScan(
    makeMessage({
      content: "free nitro click here http://sketchy-site.xyz/claim",
      authorId: USER_A,
    }),
  );
  check("scam signature KÈM link lạ → phạt", v4.length === 1 && v4[0].module === "malware");

  const v5 = await runScan(
    makeMessage({ content: "xem file này", attachments: ["virus.exe"], authorId: USER_A }),
  );
  ...
  for (let i = 0; i < 10; i++) {
    const res = await runScan(
      makeMessage({ content: `xin chào @bạn số ${i}`, mentions: 1, authorId: USER_B }),
    );
  ...
  for (let i = 0; i < 5; i++) {
    const res = await runScan(
      makeMessage({ content: `ảnh ${i}`, attachments: [`pic${i}.png`], authorId: USER_B }),
    );
    attCount += res.length;
  }
  ...
  check(
    "domain scam → phát hiện",
    findMaliciousLink("vào đây https://discord-gift.ru/claim") !== null,
  );
  check("IP trực tiếp → phát hiện", findMaliciousLink("xem http://192.168.1.5/payload") !== null);
  check(
    "youtube (lành tính) → KHÔNG phát hiện",
    findMaliciousLink("xem https://www.youtube.com/watch?v=x") === null,
  );
  check(
    "từ giveaway KHÔNG link → KHÔNG phát hiện",
    findMaliciousLink("giveaway ở server kia vui ghê") === null,
  );
  check(
    "giveaway + link github (lành tính) → KHÔNG phát hiện",
    findMaliciousLink("giveaway script https://github.com/a/b") === null,
  );
  check(
    "free nitro + link lạ → phát hiện",
    findMaliciousLink("free nitro http://lừa-đảo.xyz") !== null,
  );

  // Threat intel học được
  _setThreatIntelForTest(["credential", "infostealer"], ["free gift redeem now"]);
  check(
    "từ học được + link github → KHÔNG phạt (từ chung)",
    findLearnedThreat("password flow https://github.com/a/b") === null,
  );
  check(
    "từ học được + link LẠ → phạt",
    findLearnedThreat("credential stealer download http://evil-site.xyz/x") !== null,
  );
  check(
    "cụm từ học được → phạt cả khi không link (đặc thù)",
    findLearnedThreat("bấm vào đây free gift redeem now") !== null,
  );
  check(
    "từ học được không nằm trong tin → null",
    findLearnedThreat("chill thôi https://youtube.com/x") === null,
  );

  check(
    "link youtube là lành tính (findSuspiciousLink)",
    findSuspiciousLink("xem https://youtu.be/abc") === null,
  );
  check(
    "link domain lạ là đáng ngờ (findSuspiciousLink)",
    findSuspiciousLink("xem http://sketchy.xyz/a") !== null,
  );

  check(
    "file .png → an toàn",
    findDangerousAttachment(new Map([["a", { name: "photo.png" }]])) === null,
  );
  check(
    "file .bat → nguy hiểm",
