...
(async () => {
  ...
  // ─── 1e. First-byte timeout: upstream treo header → cắt sớm, không đứng im ─
  {
    ...
    const mock = await startRawUpstream((req, res) => setTimeout(() => {
    ...
    try {
      ...
      check("upstream treo header → proxy cắt theo first-byte rồi trả 502", res.status === 502, `${dt}ms`);
      check("cắt sớm (không đứng im hết timeout 10s)", dt < 3000, `${dt}ms`);
      check("đã thử lại đủ 3 lượt trước khi bỏ", mock.hits["/chat/completions"] === 3, `hits=${mock.hits["/chat/completions"]}`);
    } finally {
      proxy.kill();
      await stopServer(mock.server);
    }
  ...
  // ─── 1f. Idle watchdog giữa stream: gửi 1 chunk rồi im → proxy đóng sạch ───
  {
    ...
    try {
      ...
      const outcome = await new Promise((resolve) => {
        const req = http.request(
          { host: "127.0.0.1", port: PORT, path: "/chat/completions", method: "POST", headers: { authorization: "Bearer t", "content-type": "application/json" } },
          (res) => {
            let got = "";
            res.on("data", (c) => (got += c));
      ...
      check("stream im lặng giữa chừng → proxy đóng sạch, client không bị treo", outcome.how === "end", outcome.how);
      check("chunk đầu vẫn tới client trước khi bị đóng", outcome.got.includes('"xin"'), JSON.stringify(outcome.got));
    } finally {
      proxy.kill();
      await stopServer(mock.server);
    }
  ...
  // ─── 1f2. Im lặng TRƯỚC chunk đầu (ca "kẹt" thật) → hủy lượt và thử lại ────
  {
    ...
    try {
      await waitForPort(PORT);
      const res = await request(PORT, "/chat/completions");
      check("im lặng trước chunk đầu → hủy lượt, thử lại, client nhận 200", res.status === 200, `status=${res.status}`);
      check("client nhận dữ liệu lượt 2", res.body.includes("lan2"), JSON.stringify(res.body));
      check("đã gọi upstream đúng 2 lần", mock.hits["/chat/completions"] === 2, `hits=${mock.hits["/chat/completions"]}`);
    } finally {
      proxy.kill();
      await stopServer(mock.server);
    }
  ...
  // ─── 1f3. Stream chảy dài hơn first-byte KHÔNG bị cắt oan ───────────────────
  {
    ...
    try {
      ...
      const res = await request(PORT, "/chat/completions");
      const dt = Date.now() - t0;
      check("stream dài hơn first-byte vẫn nhận đủ (không cắt oan)", res.status === 200 && res.body.includes("[DONE]"), `status=${res.status}, ${dt}ms`);
      check("nhận đủ 5 chunk", (res.body.match(/"n":/g) || []).length === 5, JSON.stringify(res.body));
    } finally {
      proxy.kill();
      await stopServer(mock.server);
    }
  ...
  // ─── 1g. Ngân sách tổng: upstream treo hết → dừng theo budget, không vô tận ─
  {
    ...
    try {
      ...
      const res = await request(PORT, "/chat/completions");
      const dt = Date.now() - t0;
      check("hết ngân sách tổng → trả 502 (không retry 20 lần × 30s)", res.status === 502, `${dt}ms`);
  ...
  // ─── 1h. Ngắt mạch bán mở: lỗi liên tiếp → chặn nhanh, rồi thăm dò hồi phục ─
  {
    ...
    try {
      ...
      check("2 lỗi liên tiếp → request kế bị ngắt mạch trả 503", r1.status === 500 && r2.status === 500 && r3.status === 503, `${r1.status}/${r2.status}/${r3.status}`);
      ...
      check("hết thời gian nghỉ → thăm dò thành công, mạch đóng lại", r4.status === 200 && r5.status === 200, `${r4.status}/${r5.status}`);
    } finally {
      proxy.kill();
      await stopServer(mock.server);
    }
  ...
  check("mặc định chịu nghẽn 6 lần thử (KIRA_PROXY_RETRIES ?? 6)", src.includes("KIRA_PROXY_RETRIES ?? 6"));
  ...
  // Tăng cường 19/09/2026
  check("tách timeout chờ byte đầu (KIRA_PROXY_FIRST_BYTE_MS + firstByteTimer)",
    src.includes("KIRA_PROXY_FIRST_BYTE_MS") && src.includes("firstByteTimer"));
  check("huỷ timer first-byte ngay khi nhận header (không cắt oan stream dài)",
    src.includes("clearTimeout(firstByteTimer)") && src.includes("attemptController.abort"));
  check("idle watchdog cho stream (KIRA_PROXY_IDLE_MS + waitFirstChunk/continueStream)",
    src.includes("KIRA_PROXY_IDLE_MS") && src.includes("waitFirstChunk") && src.includes("continueStream"));
  check("im lặng trước chunk đầu thì hủy lượt và thử lại (first.idle)",
    src.includes("first.idle") && src.includes("upstream im lặng trước chunk đầu"));
  check("ngân sách tổng chặn retry cộng dồn vô tận (KIRA_PROXY_TOTAL_BUDGET_MS + deadline)",
    src.includes("KIRA_PROXY_TOTAL_BUDGET_MS") && src.includes("deadline"));
  check("ngắt mạch bán mở (breakerAllows/breakerFailure + ngưỡng env)",
    src.includes("KIRA_PROXY_BREAKER_THRESHOLD") && src.includes("breakerAllows") && src.includes("breakerFailure"));
  check("health endpoint phơi trạng thái ngắt mạch để giám sát",
    src.includes("breaker:") && src.includes("breakerFails"));
  check("thăm dò hồi phục: chỉ một request thăm dò khi mạch mở (probeInFlight)",
    src.includes("probeInFlight") && src.includes("probe:"));
  check("giải phóng kết nối lỗi có giới hạn (drain + MAX_ERROR_BODY)",
