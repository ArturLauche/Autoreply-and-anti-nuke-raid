...
chứng. Convex backend KHÔNG cần deploy tay: CI (GitHub Actions) tự
`npx convex deploy` sau mỗi push lên `main` — việc của lệnh này chỉ là phần bot.
...
4. **Restart bot** — bước này được phép tự làm vì đã qua cổng kiểm chứng ở bước 3
...
5. **Xác minh bot sống** — bắt buộc, không restart là xong đâu:
...
- Cấm (vùng 🔴): đọc `.env`/`bot/.bot-key`, `pm2 startup/system`, `reboot`.

## Báo cáo cuối

Gộp thành bảng ngắn: commit mới về | kết quả 4 lớp kiểm chứng | trạng thái
`pm2 status` | dấu hiệu sống trong log. Có vấn đề → nói rõ bước nào hỏng + kế
hoạch sửa, không tự tung biện pháp ngoài kịch bản.
