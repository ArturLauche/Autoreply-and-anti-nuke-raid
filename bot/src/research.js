 ...
 * (ms) — vd 7200000 = 2 giờ.
 */
const RESEARCH_INTERVAL_MS = Math.max(
  30 * 60 * 1000,
  Number(process.env.RESEARCH_INTERVAL_MS) || 60 * 60 * 1000,
);
...
function extractKeywords(text, maxPerSource = 10) {
  ...
  for (const w of words) {
    ...
    const suspicious = /\d/.test(clean) || /-/.test(clean) || clean.length >= 8;
...
async function researchReddit(source) {
  ...
  for (const c of posts) {
    ...
    if (
      /(scam|raid|nuke|hack|compromis|phish|malware|token|webhook|bot.?net|spam|fake|imperson)/i.test(
        title,
      )
    ) {
      phrases.push(
...
async function researchCisa(source) {
  ...
  const vulns = (data?.vulnerabilities ?? [])
    ...
    .map((v) => ({
      cve: v?.cveID ?? "",
      kw: extractKeywords(
        `${v?.vendorProject ?? ""} ${v?.product ?? ""} ${v?.shortDescription ?? ""}`,
        5,
      ),
    }))
    .filter((v) => v.cve);
...
async function aiSynthesize(researchText, previousKeywords) {
  try {
    ...
    if (
      typeof aiClient.researchAvailable === "function"
        ? !aiClient.researchAvailable()
        : !aiClient.aiAvailable()
    )
      return null;
...
Chỉ trả JSON thuần (không markdown): {"keywords": ["từ khóa scam mới", ...], "phrases": ["cụm từ scam nhiều từ", ...], "summary": "tóm tắt 2-3 câu tiếng Việt về xu hướng đe dọa mới nhất"}. Tối đa 15 keywords, 8 phrases. Bỏ từ quá phổ biến (discord, server, free...).`;
    ...
    const raw = await (
      typeof aiClient.researchChat === "function" ? aiClient.researchChat : aiClient.chatForResearch
    )(
      [
  ...
  if (intel?.aiWeeklyEnabled !== false) {
    ...
    if (newKeywords.length > 0 || cveList.length > 0 || weekElapsed || richFindings) {
      ...
      if (ai) {
        ...
        const aiNew = ai.keywords.filter(
          (k) => !oldSet.has(k.toLowerCase()) && !seen.has(k.toLowerCase()),
        );
  ...
  try {
    ...
    if (digestDue && (previousKeywords.length > 0 || newKeywords.length > 0)) {
      const digest = await buildWeeklyDigest(
        [...previousKeywords, ...newKeywords].slice(0, 40),
        newPhrases.slice(0, 8),
      );
      if (digest) {
        globalThis.__protogonLastDigest = now;
        await store.client.mutation("threatIntel:botSetResearchMeta", { digest }).catch(() => null);
        // Digest cũng tôn trọng cờ thông báo học tập (mặc định TẮT).
        if (intel?.notifyEnabled === true) {
          await postDigestToLog(store, digestClient, digest).catch(() => {});
        }
      }
    }
  } catch {
...
async function learnNow(store, requestedBy) {
  ...
  await store.client.mutation("threatIntel:requestManualLearn", { requestedBy }).catch(() => null);
  // Xóa cờ ngay — learnNow chạy trực tiếp, không cần tick nhận lại.
  await store.client.mutation("threatIntel:botClaimManualLearn", {}).catch(() => null);
    ...
    try {
      ...
      const embed = logEmbed({
        ...
        fields: [
          ...
          {
            name: "AI tổng hợp",
            value: res.aiUsed ? "✅ Có (Mimo V2.5)" : "⚙️ Không (heuristics)",
            inline: true,
          },
        ],
...
async function aiReviewKeywords(keywords) {
  ...
  try {
    ...
    return list
      ...
      .slice(0, 15)
      .map((s) => ({
        keyword: String(s.keyword).slice(0, 80),
        benignHits: Math.max(0, Math.min(100, Number(s.benignHits) || 0)),
      }));
  } catch {
    return [];
  }
...
async function postDigestToLog(store, client, digest) {
