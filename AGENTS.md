...
- [ ] `bun run test` — toàn bộ suites xanh (hiện tại **59 suites** — số liệu 22/09/2026; nếu runner báo ít hơn nhiều → có suite bị bỏ sót, điều tra trước khi kết luận xanh). Số liệu này phải khớp với `CONTRACT_SUITES` trong `.opencode/plugins/guardrails.js` — đổi suite mới phải sửa CẢ HAI chỗ trong cùng commit
- [ ] `bun run test:ts` — 5 suite tầng Convex/Haimiya viết bằng TypeScript (chạy bằng bun; tách khỏi `bun run test` để không đụng phép đo coverage c8). Đụng `convex/` hoặc `bot/src/ai.js` thì bắt buộc chạy
