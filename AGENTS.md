...
- [ ] `bun run test:ts` — 6 suite tầng Convex/Haimiya viết bằng TypeScript (chạy bằng bun; tách khỏi `bun run test` để không đụng phép đo coverage c8). Đụng `convex/` hoặc `bot/src/ai.js` thì bắt buộc chạy
